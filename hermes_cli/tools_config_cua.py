...
# Nia fork delta — P0 computer-use pin: the cua-driver binary and npm
# `@trycua/cua-driver` must be the same release, and scored runs must be
# reproducible. While set, every installer call pins to this release and the
# upstream check-update / "install latest" nudge is suppressed.
# Bump this constant when the bake-off is re-run.
...
def install_cua_driver(upgrade: bool = False, require_confirmed_update: bool = False,
    ...
    if not binary and not upgrade:
        ...
        return _run_cua_driver_installer(label="Installing", pin_version=PINNED_CUA_DRIVER_VERSION or None)
    ...
    # Compatible existing install: no download unless the binary is off the Nia
    # pin (binary 0.23.x vs npm 0.28.x was the scored-run mismatch).
    off_pin = False
    ...
    if binary and not repair_existing:
        if PINNED_CUA_DRIVER_VERSION:
            # Nia pin: never nudge past the pinned release. An on-pin binary is
            # up to date by definition; an off-pin one refreshes to the pin —
            # no check-update call, no "install latest" prompt.
            have = _parse_cua_driver_semver(_cua_driver_version(binary)) or _parse_cua_driver_semver(
                str((contract or {}).get("version") or ""))
            if have == PINNED_CUA_DRIVER_VERSION:
                _print_success(f"    {driver_cmd} is on the pinned release ({have}).")
                return True
            confirmed_version = PINNED_CUA_DRIVER_VERSION
        else:
    ...
    ok = _run_cua_driver_installer(
        ...
        pin_version=confirmed_version or (PINNED_CUA_DRIVER_VERSION or None), show_progress=show_installer_progress,
