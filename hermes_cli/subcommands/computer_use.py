    ...
    if sys.platform == "linux":  # hand-written daemon units: dead `serve` is invisible to the binary contract (#114748)
        ...
        for _kind, unit, _target, serve, socket in cua_daemon_units():
            if serve and cua_daemon_listening(path, socket) is False:
                ...
                rc = 1
    from hermes_cli.tools_config import PINNED_CUA_DRIVER_VERSION, _parse_cua_driver_semver
    have = _parse_cua_driver_semver(version)
    if PINNED_CUA_DRIVER_VERSION and have == PINNED_CUA_DRIVER_VERSION:
        # Nia pin: the driver is held at the pinned release; do not nudge past it.
        print(f"  ✓ On pinned release {have} (Nia pin).")
        return rc
    try:
