...
import platform  # Nia fork delta: Windows VAD default (see stt.vad below)
...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # Nia fork delta: ship a working vision default so vision_analyze /
        # computer_use capture work out of the box (glm-5.3-flash does
        # text+image+video; not z-ai/glm-5.3 which is text-only).
        # image_routing._explicit_aux_vision_override treats exactly this
        # block as "not user-pinned" (native attach still wins).
        "vision": _aux(120, download_timeout=30, provider="openrouter", model="z-ai/glm-5.3-flash"),
        ...
        # Nia fork delta: pin compression to glm-5.2 (text-only, 1M context;
        # not glm-5.3 / :batch) so compaction never inherits a slow main model.
        "compression": _aux(120, no_progress_timeout=None, provider="openrouter", model="z-ai/glm-5.2"),
        "skills_hub": _aux(30),
    ...
    "stt": {
        ...
        "local": {
            ...
            # Nia fork delta: Silero VAD off by default on Windows —
            # onnxruntime hard-crashes (0xC0000005) in managed venvs, which
            # kills serve mid-dictate (ECONNRESET after package+reinstall).
    ...
    "wake_word": {
        ...
        # Nia fork delta: sherpa (free, ANY typed phrase, no training) is the
        # default engine so the "Ok Nia" phrase works without a trained model.
        "provider": "sherpa",
