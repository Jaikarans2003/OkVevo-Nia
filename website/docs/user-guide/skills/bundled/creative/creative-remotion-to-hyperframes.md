---
title: "Remotion To Hyperframes — Port an existing Remotion (React) composition's source to HyperFrames HTML"
sidebar_label: "Remotion To Hyperframes"
description: "Port an existing Remotion (React) composition's source to HyperFrames HTML"
---
...
| Path | `skills/creative/remotion-to-hyperframes` |
...
Run [`scripts/lint_source.py`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/scripts/lint_source.py) over the Remotion source directory. The lint detects patterns that can't translate cleanly:
...
If any blocker fires, **stop**. Read [`references/escape-hatch.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/escape-hatch.md) and surface the recommendation message. Warnings don't stop translation — drop the offending construct in step 3 and note the gap in `TRANSLATION_NOTES.md`. `@remotion/lambda` config is the canonical warning case: the skill drops the import + `renderMediaOnLambda(...)` calls but translates the rest of the composition.

### Step 2: Plan the translation

Read [`references/api-map.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/api-map.md) — the index of every Remotion API and its HF equivalent or per-topic reference. Identify which topic references you'll need based on what the source uses:

| Source contains                                                           | Load reference                                |
| ------------------------------------------------------------------------- | --------------------------------------------- |
| `Composition`, `defaultProps`, `schema`, `calculateMetadata`              | [`parameters.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/parameters.md)   |
| `Sequence`, `Series`, `Loop`, `AbsoluteFill`, `Freeze`                    | [`sequencing.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/sequencing.md)   |
| `useCurrentFrame`, `interpolate`, `spring`, `Easing`, `interpolateColors` | [`timing.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/timing.md)           |
| `Audio`, `Video`, `Img`, `IFrame`, `staticFile`, `delayRender`            | [`media.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/media.md)             |
| `TransitionSeries`, `@remotion/transitions`                               | [`transitions.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/transitions.md) |
| `@remotion/lottie`                                                        | [`lottie.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/lottie.md)           |
| `@remotion/google-fonts/<Family>`, `Font.loadFont`, `@font-face`          | [`fonts.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/fonts.md)             |
...
Custom React subcomponents inline as repeated HTML using the prop interface as the template (see [`parameters.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/parameters.md) for the per-instance `data-*` pattern).

### Step 4: Validate

Run the eval harness — [`references/eval.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/eval.md) for the full guide. Quick path:
...
Threshold: ~0.02 below `p05` of the source's complexity tier (see `eval.md`'s validated thresholds table). If the diff fails, run [`scripts/frame_strip.sh`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/scripts/frame_strip.sh) to see _which_ frames diverged, then re-read the relevant timing/sequencing/media reference.
...
Anything that didn't translate cleanly (volume ramps dropped, custom presentations approximated, fonts substituted) gets a `TRANSLATION_NOTES.md` written next to the HF output. See [`references/limitations.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/limitations.md) for the format.
...
(`@remotion/lambda` is _not_ a blocker — Lambda config is deployment, not animation. The skill drops it as a warning and translates the rest. See [`references/escape-hatch.md`](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/remotion-to-hyperframes/references/escape-hatch.md).)
