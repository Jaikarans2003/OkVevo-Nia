...
export const host = {
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Nia.')
    }
  ...
  agents: async () => {
    ...
    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Nia.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Nia to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Nia connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Nia unavailable')
    }
...
/** The oversized Ubuntu Bold lettering an empty chat is titled with — core writes
 *  "NIA" with it, a `chat.empty` contribution writes its own name. */
