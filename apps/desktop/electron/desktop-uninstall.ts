 ...
 *
 *   macOS:   …/Nia.app/Contents/MacOS/Nia  → …/Nia.app
 *   Windows: …\Nia\Nia.exe                 → …\Nia  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Nia.app/Contents/MacOS/Nia → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Nia.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Nia.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/](Nia|Hermes)$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/Nia
  const dir = p.dirname(exe)
