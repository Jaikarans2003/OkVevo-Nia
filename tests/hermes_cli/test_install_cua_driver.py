...
class TestInstallCuaDriverUpgrade:
    ...
    def test_non_upgrade_off_pin_refreshes_to_pinned_release(self):
        """Nia pin: a compatible but off-pin binary refreshes to the pin."""
        from unittest.mock import MagicMock
    ...
    def test_upgrade_on_pin_skips_check_update(self):
        """Nia pin: an on-pin binary never triggers the check-update /
        install-latest flow, even on explicit --upgrade."""
        from unittest.mock import MagicMock
        ...
        with patch.object(tools_config.shutil, "which",
             ...
             patch(
                 "tools.computer_use.cua_backend_driver.cua_driver_update_check",
                 side_effect=AssertionError("check-update must not run when pinned"),
             ), \
             patch.object(tools_config, "_run_cua_driver_installer",
                          return_value=True) as runner, \
             patch(
                 "subprocess.run",
                 return_value=MagicMock(stdout=f"cua-driver {pinned}", returncode=0),
             ), \
             patch.object(tools_config, "_print_success"), \
             patch.object(tools_config, "_print_info"):
            assert tools_config.install_cua_driver(upgrade=True) is True
...
class TestRequireConfirmedUpdate:
    ...
    def _install(self, check_state, require_confirmed, contract_status=None,
        ...
        # Exercise the upstream (unpinned) confirmed-update flow: Nia's pin
        # short-circuits check-update, so these tests run with the pin off.
        with patch.object(tools_config, "PINNED_CUA_DRIVER_VERSION", ""), \
