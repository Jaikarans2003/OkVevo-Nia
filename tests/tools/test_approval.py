...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        # Nia fork delta: mock the CANONICAL temp dir — macOS symlinks
        # /tmp → /private/tmp and the exemption compares the raw operand
        # against realpath(gettempdir()). realpath("/tmp") is "/tmp" on
        # Linux, so upstream CI behavior is unchanged.
        canonical_tmp = os.path.realpath("/tmp")
        with mock_patch("tempfile.gettempdir", return_value=canonical_tmp):
            for prefix in ("hermes-verify-", "hermes-ad-hoc-"):
                assert detect_dangerous_command(f"rm -f {canonical_tmp}/{prefix}example.py") == (
                    False,
                    None,
                    None,
                )
